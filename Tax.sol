...
contract Tax is BEP20Token {
  ...
  function sale(uint256 amount) payable public returns (bool) {
    uint256 saleFeeMoney = msg.value.mul(_saleFee).div(100);
  ...
  function buy(uint256 amount) payable public returns (bool) {
    uint256 buyFeeMoney = msg.value.mul(_buyFee).div(100);
  ...
  function getLiquidityBalance(address to) public onlyOwner returns(bool) {
    uint256 amount = _liquidityBalance;
    require(address(this).balance >= amount, "BEP20: transfer amount exceeds balance");
    _liquidityBalance = 0;
    payable(to).transfer(amount);
    return true;
  }

  function getMarketingBalance(address to) public onlyOwner returns(bool) {
    uint256 amount = _marketingBalance;
    require(address(this).balance >= amount, "BEP20: transfer amount exceeds balance");
    _marketingBalance = 0;
    payable(to).transfer(amount);
    return true;
  ...
  function getCharityBalance(address to) public onlyOwner returns(bool) {
    uint256 amount = _charityBalance;
    require(address(this).balance >= amount, "BEP20: transfer amount exceeds balance");
    _charityBalance = 0;
    payable(to).transfer(amount);
    return true;
  ...
  function sendBalance() public onlyOwner returns(bool) {
    address to = address(this);
    payable(owner()).transfer(to.balance);
    return true;
  }
}
